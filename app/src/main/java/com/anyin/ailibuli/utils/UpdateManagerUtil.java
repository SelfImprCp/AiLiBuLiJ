...
import com.anyin.ailibuli.interf.ICallbackResult;
import com.anyin.ailibuli.res.UpdateRes;
import com.anyin.ailibuli.server.DownloadService;
import com.anyin.ailibuli.ui.dialog.BasicDialog;
import com.anyin.ailibuli.ui.dialog.CommonDialog;
import com.anyin.ailibuli.ui.dialog.DialogHelper;
import com.anyin.ailibuli.ui.dialog.WaitDialog;
...
public class UpdateManagerUtil {
	...
	/**
	 * 
	 * 1.用法 new UpdateManagerUtil(SettingActivity.this, true,
	 * true).checkUpdate();
	 * 
	 * 2.在checkUpdate 中调用 服务器，获取最新版本，
	 * 
	 * 
	 * 3.清单文件中配服务 <service
	 * android:name="cn.mofox.business.server.DownloadService" />
	 * 
	 * 
	 */
	...
	private void checkUpdate(final Context context) {
		if (!StringUtils.toBool(SharePreferencesUitl.getSharePreferencesUitl(context).get(AppConfig.KEY_CHECK_UPDATE))) {
			return;
		}
		...
		handler.postDelayed(new Runnable() {
			...
			@Override
			public void run() {
				new UpdateManagerUtil(context, false, false).checkUpdate();
			}
		}, 2000);
	}
	...
	public static void openApp(Context context, String packageName) {
		Intent mainIntent =  AppContext.getInstance() .getPackageManager()
